...
def _get_db_path():
    ...
    try:
        ...
        return os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), 'data.db')
...
def create_with_script(title: str, topic: str, script_content: str = '') -> dict:
    """一步创建剧集并保存剧本（原子操作）"""
    conn = get_db()
    row = conn.execute("SELECT COALESCE(MAX(episode_num), 0) + 1 FROM episodes").fetchone()
    next_num = row[0]
    now = datetime.now().isoformat()
    status = 'completed' if script_content else 'draft'

    conn.execute(
        "INSERT INTO episodes (episode_num, title, topic, script_content, status, updated_at) "
        "VALUES (?, ?, ?, ?, ?, ?)",
        (next_num, title, topic, script_content, status, now),
    )
    conn.commit()
    ep_id = conn.execute("SELECT last_insert_rowid()").fetchone()[0]
    conn.close()
    return {"id": ep_id, "episode_num": next_num, "title": title, "topic": topic, "status": status}
